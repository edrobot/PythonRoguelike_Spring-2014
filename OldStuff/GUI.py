...
def render_all():
    ...
    if GameState.fov_recompute:
        ...
        for y in range(cfg.MAP_HEIGHT):
            for x in range(cfg.MAP_WIDTH):
                ...
                wall = GameState.map[x][y].block_sight
                if not visible:
                    #if it's not visible right now, the player can only see it if it's explored
                    if GameState.map[x][y].explored:
                        if wall:
                    ...
                    GameState.map[x][y].explored = True
    ...
    libtcod.console_print_ex(cfg.panel, 1, 3, libtcod.BKGND_NONE, libtcod.LEFT, 'Dungeon level ' + str(GameState.dungeon_level))
